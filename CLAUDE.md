...
```text
~/.venvs/aidemo2026    notebook / 学生用 slim 環境 (aidemo2026 umbrella 共通環境、旧 dfs2026 を統合。厳密管理、原則変更不可)
...
```text
requirements.txt       aidemo2026 の direct dependencies (slim、学生配布想定。aidemo2026 は umbrella 共通環境)
...
学生に配布する想定の slim venv (aidemo2026 umbrella 共通環境)。再現性とパッケージ最小性のため **厳密管理**:
...
- `~/.venvs/aidemo2026` (学生用 slim venv) で動くことを目指す。
