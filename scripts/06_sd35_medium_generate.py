# 06_sd35_medium_generate.py
#
# Stable Diffusion 3.5 Medium の生成 script (HF gated)。
# 初回利用には https://huggingface.co/stabilityai/stable-diffusion-3.5-medium の規約承認と
# `hf auth login` が必要なことに注意。
#
# 設定: scripts/diffusers_probe.json
#   common.{prompt, negative_prompt, seed, width, height}
#   models.sd35_medium.{model_id, num_inference_steps=28, guidance_scale=4.5,
...
MODEL_KEY = "sd35_medium"
SCRIPT_NAME = "06_sd35_medium_generate.py"
OUTPUT_BASENAME = "sd35_medium_generate"
...
def main() -> int:
    ...
    print("=== diffusers_probe / SD3.5 Medium generate ===")
    ...
    print()
    print("[note] SD3.5 Medium は HF gated repo です。事前に下記を完了してください:")
    print("       1) https://huggingface.co/stabilityai/stable-diffusion-3.5-medium で利用条件を承認")
    print("       2) ターミナルで `hf auth login` を実行")
    print("[note] 初回 download は数 GB ~ 十数 GB あり、download 時間が混ざります。")
    print()

    try:
        from diffusers import StableDiffusion3Pipeline  # pyright: ignore[reportPrivateImportUsage]
    except Exception as e:
        print(f"[error] diffusers の import に失敗: {type(e).__name__}: {e}")
        print("[hint ] SD3 は比較的新しい diffusers が必要です (>= 0.30)。")
        return 1

    print("[load] StableDiffusion3Pipeline.from_pretrained ...")
    ...
    try:
        ...
        if fb:
            ...
            try:
                ...
                print("[gpu-server-candidate] SD3.5 Medium は MPS では実用厳しい可能性 → GPU サーバー候補")
                traceback.print_exc()
                return 1
        else:
            traceback.print_exc()
